...
// When the opener is clicked (large button), show the chat window and hide the opener
openBtn.addEventListener("click", () => {
    chatWindow.classList.remove("hidden");
    openBtn.classList.remove("large-opener");
    // focus the input for convenience
    setTimeout(() => { userInput.focus(); }, 50);
});

// When the close button is clicked, hide the chat window and show a large reopen button
closeBtn.addEventListener("click", () => {
    chatWindow.classList.add("hidden");
    openBtn.classList.add("large-opener");
});
